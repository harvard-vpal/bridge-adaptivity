...
from .models import OutcomeService
...
def store_outcome_parameters(request_params, sequence, lti_consumer):
    ...
    An LTI consumer that does not send that parameter does not expect scoring updates for that particular request.
    """
    ...
    if result_id:
        ...
        # FIXME(wowkalucky): temporary dev switch
        result_service = result_service.replace('localhost', 'book.local')
...
def send_score_update(sequence, score):
    ...
    That way we can manually fix things up on the campus system later if necessary.
    """
